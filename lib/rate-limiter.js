...
import { getHoursUntilReset } from "@/lib/cron-scheduler";
...
async function checkAuthenticatedUserLimits(userId) {
  ...
  if (user.usage.messagesToday >= limits.messagesPerDay) {
    ...
    return {
      allowed: false,
      error: `Message limit exceeded (${limits.messagesPerDay} messages). Resets in ${hoursUntilReset} hours.`,
      resetTime: resetTime,
      usage: {
...
function checkUnauthenticatedUserLimits(request) {
  ...
  if (!usage || now > usage.resetTime) {
    ...
    // Set 12-hour reset for unauthenticated users
    const resetTime = new Date(now);
    const currentHour = resetTime.getHours();
  ...
  if (usage.messagesToday >= limits.messagesPerDay) {
    ...
    return {
      allowed: false,
      error: `Message limit exceeded for unauthenticated users (${limits.messagesPerDay} messages). Please sign in for higher limits or wait ${hoursUntilReset} hours.`,
